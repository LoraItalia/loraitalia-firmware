...
2. git checkout master -- /path/to/file
You pull in the file from the master branch into your working directory. This means:

You’re not switching branches again; you remain on patches.
You’re just copying the version of /path/to/file from master into your current working tree.
At this point, your working directory has the file as it exists in master, but your active branch is still patches.
...
This command compares master (old) vs. patches (new) but only for the specific file.
...
…to restore the file from master again or just revert it to the version that was in patches before you overwrote it. But rm is fine if the file didn’t originally exist in patches.
...
to add the changes, you can reverse them with:
